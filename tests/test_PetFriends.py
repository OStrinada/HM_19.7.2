...
def test_get_api_key_for_invalid_user(email=invalid_email, password=valid_password):
    """Проверяем, что запрос API ключа от несуществующего пользователя не возвращает
    статус 200"""
...
@pytest.mark.xfail
def test_add_new_pet_with_invalid_age(name='Marsy', animal_type='cat',
                                      age=invalid_age, pet_photo='images/CAT2.jpg'):
...
def test_successful_update_self_pet_info(name='Gerry', animal_type='cat', age='5'):
...
def test_create_new_pet_without_photo_valid_data(name='SOBR', animal_type='cat', age='7'):
    """Проверяем, что можно добавить питомца с корректными данными без фоторгафии"""
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Mars", "cat", "5", "images/CAT2.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
